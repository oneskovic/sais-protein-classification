...
def zero_padd(x, max_len, return_as_tensors=False):
    n = len(x[0][0])
    if return_as_tensors:
        x_padded = torch.zeros((len(x), max_len, n))
    else:
        x_padded = np.zeros((len(x), max_len, n))
...
def preprocess_encode_ngram(data, ngram_len = 3):
    ...
    if 'prot_Pfam' in data.columns:        
        ...
        label_dict = label_dict_small
        is_in_set = np.array([x in label_dict_small for x in labels])
        if not np.all(is_in_set):
            label_dict = label_dict_large
        encoded_labels = np.array([label_dict[label] for label in labels])
        return encoded_seqs, encoded_labels
    else:
        return encoded_seqs

def preprocess_encode_onehot(data, include_len = False, return_as_tensors = False):
    aa_seqs = data['AA_sequence']
    ...
    distinct_aas = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'Y']
    codes_dict = make_dict(distinct_aas)
    encoded_seqs = zero_padd([onehot_encode_aa(seq, codes_dict, return_as_tensors) for seq in aa_seqs], max_len, return_as_tensors)
    if(include_len):
    ...
    if 'prot_Pfam' in data.columns:        
        ...
        if not np.all(is_in_set):
            label_dict = label_dict_large
        if return_as_tensors:
            encoded_labels = torch.tensor([label_dict[label] for label in labels], dtype=torch.long)
        else:
            encoded_labels = np.array([label_dict[label] for label in labels])
        return encoded_seqs, encoded_labels
    else:
